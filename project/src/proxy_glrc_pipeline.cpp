                                                   ...
                                                   proxy_proto::RecoveryReply *response)
{
  ...
  try
  {
    ...
    const double ingress_floor =
        node_block_transfer_seconds(static_cast<size_t>(block_size), m_sys_config->NodeBlockBandwidthMBps);
                                         ...
                                         proxy_proto::RecoveryReply *response)
{
  ...
  for (int i = 0; i < f; i++)
  {
    ...
    // recovery_size==0 selects one full-block stream on the datanode.
    ws->recovery_info.set_recovery_offset(0);
    ws->recovery_info.set_recovery_size(0);
  ...
  const double hub_ingress_floor =
      node_block_transfer_seconds(static_cast<size_t>(block_size) * static_cast<size_t>(hub_chain_n),
