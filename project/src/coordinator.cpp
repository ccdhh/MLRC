  ...
  int allocate(const std::string &proxy_ip, int proxy_grpc_port, bool hub_band, int chain_id, int hop_index)
  {
    ...
    for (int attempt = 0; attempt < half; ++attempt)
    {
      ...
      if (s_inflight_ports[proxy_key].count(port) > 0)
        continue;
      auto cooldown_it = s_port_cooldown_until[proxy_key].find(port);
      if (cooldown_it != s_port_cooldown_until[proxy_key].end())
      {
        if (cooldown_it->second > now)
          continue;
        s_port_cooldown_until[proxy_key].erase(cooldown_it);
      }
...
std::unordered_map<std::string, std::unordered_map<int, std::chrono::steady_clock::time_point>>
    GlrcPipelinePortAllocator::s_port_cooldown_until;
...
{
  std::lock_guard<std::mutex> lock(s_alloc_mutex);
  const auto cooldown_until = std::chrono::steady_clock::now() + std::chrono::seconds(30);
  const char *one_proxy_per_host = std::getenv("DDRT_ONE_PROXY_PER_HOST");
  const bool preserve_reservation =
      one_proxy_per_host != nullptr && one_proxy_per_host[0] != '\0' && one_proxy_per_host[0] != '0';
  auto release_one = [&](const ECProject::GlrcPipelineChainPlan &chain, const std::string &sink_ip,
                         int sink_port) {
    ...
    for (size_t hi = 0; hi < chain.hops.size() && hi < cp_it->second.hop_listen_ports.size(); ++hi)
    {
      ...
      if (!preserve_reservation)
      {
        ...
        s_port_cooldown_until[key][cp_it->second.hop_listen_ports[hi]] = cooldown_until;
      }
    }
    const std::string sink_key = sink_ip + ":" + std::to_string(sink_port);
    if (!preserve_reservation)
    {
      s_inflight_ports[sink_key].erase(cp_it->second.hub_listen_port);
      s_port_cooldown_until[sink_key][cp_it->second.hub_listen_port] = cooldown_until;
    }
  };
  for (const ECProject::GlrcPipelineChainPlan &chain : pipeline_plan.hub_chains)
    release_one(chain, hub_proxy_ip, hub_proxy_port);
  // Warmup/local-direct trials must release too, or ports stay reserved across trials.
  for (const ECProject::GlrcPipelineChainPlan &chain : pipeline_plan.local_direct_chains)
    release_one(chain, chain.local_direct_sink_proxy_ip, chain.local_direct_sink_proxy_port);
}
                                ...
                                int global_shard_begin = 0, int global_shard_count = 0)
{
  ...
  // Local single-fail chains use the failed block's proxy as sink R; reuse the
  // hub listen-port / connect-to-hub path so chain_head/hop_server stay unchanged.
  if (chain.local_direct && !chain.local_direct_sink_proxy_ip.empty())
  {
    req.set_pipeline_hub_proxy_ip(chain.local_direct_sink_proxy_ip);
    req.set_pipeline_hub_proxy_port(chain.local_direct_sink_proxy_port);
  }
  else
  {
  ...
  if (chain_ports != nullptr)
  {
    ...
    if ((role == ECProject::GlrcPipelineRole::READY ||
         role == ECProject::GlrcPipelineRole::HOP_SERVER) &&
        my_hop_index >= 0 &&
...
namespace ECProject
{
  ...
  void CoordinatorImpl::notify_proxies_ready(const proxy_proto::AppendStripeDataPlacement &plan)
  {
    ...
    std::string chosen_proxy = m_cluster_table[plan.cluster_id()].proxy_ip + ":" + std::to_string(m_cluster_table[plan.cluster_id()].proxy_port);
    {
      ...
      // Mark this generation pending before the proxy can finish and report it.
      // Otherwise checkCommitAbort may observe an older committed generation
                                                            ...
                                                            bool share_proxy_node_bandwidth)
  {
    ...
    auto run_partition = [&](int pi) {
      ...
      if (proxy_it == m_proxy_ptrs.end() || !proxy_it->second)
      {
        ...
        outcomes[pi].error = "proxy not found: " + chosen_proxy;
    ...
    if (f >= 2)
    {
      ...
      for (int pi = f - 1; pi >= 1; pi--)
      {
        ready_threads.emplace_back([&, pi]() {
          if (!run_partition_ready(pi))
          {
            ...
            std::lock_guard<std::mutex> lock(ready_error_mutex);
            ready_errors.push_back("p" + std::to_string(pi) + " acceptor READY failed");
          }
        });
      }
      ...
      if (ready_failed.load())
      {
        ...
        recovery_reply->set_success(false);
        std::string detail = "phase2 exchange acceptor READY failed";
        for (const std::string &error : ready_errors)
          detail += "; " + error;
    ...
    for (int pi = 0; pi < f; pi++)
    {
      ...
      // start its peer listener, receive the other partitions' decoded shards,
      // and publish the reconstructed [0,p) prefix. Skipping it makes peers
      // fail with connection refused whenever p < f.
    ...
    if (!all_ok)
    {
      std::string detail = "one or more phase2 partition recoveries failed";
      for (int pi = 0; pi < f; pi++)
      {
        if (!outcomes[pi].ok && !outcomes[pi].error.empty())
          detail += "; p" + std::to_string(pi) + ": " + outcomes[pi].error;
      }
      recovery_reply->set_success(false);
      recovery_reply->set_message(detail);
      return false;
                                                            ...
                                                            double *out_local_direct_hot_wall_sec)
  {
    ...
    auto log_pipeline_chain = [&](const GlrcPipelineChainPlan &chain, const char *kind) {
      std::cout << "[Coordinator] pipeline " << kind << " chain=" << chain.chain_id
                << " eq=" << chain.equation_name << " tail=" << chain.hub_is_chain_tail << " hops=";
      for (size_t hi = 0; hi < chain.hops.size(); hi++)
      {
        if (hi > 0)
          std::cout << "->";
        std::cout << glrc_block_label(chain.hops[hi].block_id, ck, cr, cz)
                  << "@" << chain.hops[hi].proxy_ip;
      }
      std::cout << std::endl;
    };
    for (const GlrcPipelineChainPlan &chain : pipeline_plan.hub_chains)
      log_pipeline_chain(chain, "hub");
    for (const GlrcPipelineChainPlan &chain : pipeline_plan.local_direct_chains)
      log_pipeline_chain(chain, "local");
    ...
    auto allocate_chain_ports = [&](const GlrcPipelineChainPlan &chain, const std::string &sink_ip, int sink_port,
      ...
      for (size_t hi = 0; hi < chain.hops.size(); ++hi)
      {
        ...
        if (hop_port <= 0)
        {
          recovery_reply->set_message(std::string("pipeline port allocation failed for hop proxy ") + hop.proxy_ip);
          return false;
        }
        cp.hop_listen_ports.push_back(hop_port);
      }
      cp.hub_listen_port = port_alloc.allocate_hub_port(sink_ip, sink_port, chain.chain_id);
      if (cp.hub_listen_port <= 0)
      {
        recovery_reply->set_message(std::string("pipeline port allocation failed for ") + sink_tag + " proxy " +
                                    sink_ip);
        return false;
      }
    ...
    for (const GlrcPipelineChainPlan &chain : pipeline_plan.hub_chains)
    {
      if (!allocate_chain_ports(chain, pipeline_plan.hub_proxy_ip, pipeline_plan.hub_proxy_port, "hub"))
        return false;
    }
    for (const GlrcPipelineChainPlan &chain : pipeline_plan.local_direct_chains)
    {
...

    {
      ...
      for (const GlrcPipelineChainPlan &chain : pipeline_plan.local_direct_chains)
      {
        ...
        if (!check_port(chain.local_direct_sink_proxy_ip, chain.local_direct_sink_proxy_port,
                        cp_it->second.hub_listen_port, "local sink"))
          return false;
      }
    }

    if (!pipeline_plan.hub_chains.empty() || !pipeline_plan.local_direct_chains.empty())
    {
      ...
      auto launch_chain_hop_ready = [&](const GlrcPipelineChainPlan &chain) {
      ...
      for (const GlrcPipelineChainPlan &chain : pipeline_plan.hub_chains)
        launch_chain_hop_ready(chain);

      // Local-chain hop READY only. Sink R binds in LOCAL_DIRECT (avoids sharing
      // hub_acceptors with the global hub on the same epoch).
      for (const GlrcPipelineChainPlan &chain : pipeline_plan.local_direct_chains)
        launch_chain_hop_ready(chain);
      ...
      std::cout << "[Coordinator] gLRC Pipeline READY ok epoch=" << exchange_epoch
                << " hub_chains=" << pipeline_plan.hub_chains.size()
                << " local_direct_chains=" << pipeline_plan.local_direct_chains.size() << std::endl;
    }
    ...
    std::cout << "[Coordinator] gLRC Pipeline data-plane start epoch=" << exchange_epoch << std::endl;
    if (!pipeline_plan.hub_chains.empty())
    {
    ...
    auto launch_chain_hops_and_head = [&](const GlrcPipelineChainPlan &chain) {
      const int last_hop_server =
          chain.hub_is_chain_tail ? static_cast<int>(chain.hops.size()) - 2
                                  : static_cast<int>(chain.hops.size()) - 1;
      for (int hi = last_hop_server; hi >= 1; hi--)
      ...
      if (!chain.hops.empty())
      {
        ...
        if (cp_it != chain_ports.end())
          cp = &cp_it->second;
        fill_pipeline_chain_fields(req, chain, pipeline_plan, local_shard_count, cz, GlrcPipelineRole::CHAIN_HEAD, 0,
                                   cp, pipeline_global_begin, global_shard_count);
    ...
    for (const GlrcPipelineChainPlan &chain : pipeline_plan.hub_chains)
      launch_chain_hops_and_head(chain);

    // Local single-fail: sink R receives the chain stream (recovered block) and writebacks.
    for (const GlrcPipelineChainPlan &chain : pipeline_plan.local_direct_chains)
    {
      const std::string sink_key =
          chain.local_direct_sink_proxy_ip + ":" + std::to_string(chain.local_direct_sink_proxy_port);
      proxy_proto::RecoveryRequest sink_req = make_base_request();
      const GlrcPipelineChainPorts *cp = nullptr;
      const auto cp_it = chain_ports.find(chain.chain_id);
      if (cp_it != chain_ports.end())
        cp = &cp_it->second;
      fill_pipeline_chain_fields(sink_req, chain, pipeline_plan, local_shard_count, cz,
                                 GlrcPipelineRole::LOCAL_DIRECT, 0, cp, pipeline_global_begin, global_shard_count);
      launch_pipeline_rpc(sink_key, sink_req);
      launch_chain_hops_and_head(chain);
    }
    ...
    {
      const PipelineRpcResult &res = results[result_index];
      if (!res.ok)
      {
        if (!rpc_errors.empty())
          rpc_errors += "; ";
        rpc_errors += "rpc[" + std::to_string(result_index) + "]: " +
                      (res.error.empty() ? "unknown failure" : res.error);
        continue;
      }
      ...
      if (res.role == static_cast<int>(GlrcPipelineRole::HUB))
        hub_egress_wall = std::max(hub_egress_wall, role_network_wall);
      else if (res.role == static_cast<int>(GlrcPipelineRole::LOCAL_DIRECT))
        local_direct_hot_wall = std::max(local_direct_hot_wall, role_network_wall);
    }
    if (!rpc_errors.empty())
    {
      recovery_reply->set_message("pipeline rpc failures: " + rpc_errors);
      recovery_reply->set_success(false);
      return false;
    }
    ...
    if (out_hub_egress_wall_sec)
      *out_hub_egress_wall_sec = hub_egress_wall;
    if (out_local_direct_hot_wall_sec)
      *out_local_direct_hot_wall_sec = local_direct_hot_wall;
                                                           ...
                                                           coordinator_proto::RecoveryReply *recovery_reply)
  {
    ...
    else
    {
      std::cout << "[Coordinator] gLRC Hybrid plan: f=" << f
                ...
                << (p < f ? " (p<f: some failed blocks get 0 phase2 shards)" : "")
    ...
    while (true)
    {
      // Each retry gets fresh pipeline exchange ports. Reusing an epoch can
      // collide with sockets from the preceding complete repair attempt.
      if (attempt > 0)
      {
        const int retry_epoch = static_cast<int>(g_glrc_pipeline_exchange_epoch.fetch_add(1));
        GlrcPipelinePlan retry_plan;
        ...
        pipeline_plan = std::move(retry_plan);
      }

      reply_phase2.Clear();
      reply_pipeline.Clear();
      double orch_phase2 = 0.0;
      double orch_pipeline = 0.0;
      double pipeline_hub_egress_wall = 0.0;
      double local_direct_hot_wall = 0.0;
      ...
      if (candidate_p == 0)
      {
        ...
        recovery_glrc_ilp_pipeline_breakdown_ex(stripe_id, failed_block_ids, &reply_pipeline, &plan_lf,
                                                &pipeline_plan, hub_block_id, 0, global_S, false, &orch_pipeline,
                                                &pipeline_hub_egress_wall, &local_direct_hot_wall);
      }
      else
      {
      ...
      {
        recovery_reply->set_success(false);
        recovery_reply->set_message(std::string("hybrid half failed at p=") + std::to_string(candidate_p) +
                                    ": phase2=" + reply_phase2.message() +
      ...
      // combined critical path is the slower observed completion on that
      // shared timeline. Balance it against the hub's shared egress path,
      // rather than comparing the two whole recovery subjobs.
      ...
      {
        recovery_reply->set_success(false);
        recovery_reply->set_message("hybrid candidate model failed at p=" + std::to_string(candidate_p) +
                                    ": " + candidate_model.error_message);
        return false;
      }
      ...
      std::cout << "[Coordinator] gLRC Hybrid adaptive candidate p=" << candidate_p
                ...
                << "s model_failed_node_hot=" << model_failed_node_hot
                << "s model_hub_egress_hot=" << model_hub_egress_hot
                << "s phase2_wall=" << phase2_wall << "s pipeline_wall=" << pipeline_wall
                << "s data_plane=" << candidate_data_plane << "s" << std::endl;

      if (candidate_data_plane < best_data_plane_sec)
      {
        best_data_plane_sec = candidate_data_plane;
        best_p = candidate_p;
      ...
      const double hotspot_max = std::max(failed_node_hot_wall, pipeline_hub_egress_wall);
      const double hotspot_gap_ratio =
          hotspot_max > 0.0 ? std::fabs(failed_node_hot_wall - pipeline_hub_egress_wall) / hotspot_max : 0.0;
      const int measured_direction =
          failed_node_hot_wall > pipeline_hub_egress_wall ? -1 : 1;
      if (hotspot_gap_ratio <= kAdaptiveHotspotGapRatio)
        break;
      if (search_direction == 0)
        search_direction = measured_direction;
      else if (measured_direction != search_direction)
        break; // crossed the measured hotspot balance point

      const int next_p = candidate_p + search_direction;
      if (next_p <= 0 || next_p >= global_S)
        break;
      p = next_p;
      attempt++;
    }
    ...
    p = best_p;
    reply_phase2 = best_reply_phase2;
    reply_pipeline = best_reply_pipeline;
    ...
    else
    {
      for (const auto &eq : plan_ilp.selected_equations)
        recovery_reply->add_selected_equations("phase2:" + eq);
      for (const auto &eq : plan_lf.selected_equations)
        recovery_reply->add_selected_equations("pipeline:" + eq);
                                    ...
                                    coordinator_proto::RepIfSuccess *reply)
  {
    ...
    if (opp == SET || opp == APPEND)
    {
      while (m_object_updating_table.find(key) != m_object_updating_table.end())
        cv.wait(lck);
    }
    else if (opp == DEL)
    {
    ...
    reply->set_ifcommit(m_object_commit_table.find(key) != m_object_commit_table.end());
    return grpc::Status::OK;
  }
